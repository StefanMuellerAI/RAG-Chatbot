/**
 * Verschluesselung fuer Geheimnisse, die in Redis liegen — derzeit die
 * API-Keys der Modellanbieter.
 *
 * AES-256-GCM ueber die Web-Crypto-API; der Schluessel wird per HKDF aus
 * AUTH_SECRET abgeleitet, damit kein zweites Geheimnis verwaltet werden muss.
 * Konsequenz, die im README steht: wird AUTH_SECRET gewechselt, sind die
 * gespeicherten Keys nicht mehr lesbar und muessen im Admin neu eingegeben werden.
 *
  ...
  try {
    ...
    throw new Error(
      "Der gespeicherte Wert laesst sich nicht entschluesseln. " +
        "Wurde AUTH_SECRET geaendert? Dann den API-Key im Admin neu eingeben.",
    );
  }
}
