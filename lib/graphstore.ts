...
export function graphName(collectionId: string): string {
  if (!/^[A-Za-z0-9_-]{1,64}$/.test(collectionId)) throw new ValidationError("Ungueltige Sammlungs-ID.");
...
function istLeererGraph(error: unknown): boolean {
  return /empty key|unknown graph|does not exist/i.test(error instanceof Error ? error.message : String(error));
}
...
/** Alle Graphen dieser App — auch verwaiste, die keiner Sammlung mehr zugeordnet sind. */
export async function deleteAllGraphs(knownCollectionIds: string[]): Promise<number> {
  const client = await getClient();
  const vorhanden = (await client.list()).filter((name) => name.startsWith(GRAPH_PREFIX));
  const ziele = new Set([...vorhanden, ...knownCollectionIds.map(graphName)]);

  let removed = 0;
  for (const name of ziele) {
    try {
      await client.selectGraph(name).delete();
      removed++;
    } catch (error) {
      if (!istLeererGraph(error)) throw error;
    }
...
export function prepareReadOnlyCypher(cypher: string): string {
  ...
  if (trimmed.includes(";")) throw new ValidationError("Bitte genau ein Cypher-Statement ohne Semikolon.");
...
export async function runReadOnlyCypher(collectionId: string, cypher: string): Promise<CypherResult> {
  const graph = await graphOf(collectionId);
  ...
  try {
    ...
    throw new ValidationError(`Cypher-Fehler: ${error instanceof Error ? error.message : String(error)}`);
  }
...
function kompakt(value: unknown): unknown {
  ...
  if (typeof value === "string") return value.length > CELL_MAX_CHARS ? `${value.slice(0, CELL_MAX_CHARS)}…` : value;
  ...
  if (typeof record.relationshipType === "string") {
    return { type: "edge", relationshipType: record.relationshipType, properties: kompaktProperties(record.properties) };
  }
  return kompaktProperties(value as Record<string, unknown>);
}

function kompaktProperties(properties: Record<string, unknown> | undefined): Record<string, unknown> {
  return Object.fromEntries(Object.entries(properties ?? {}).map(([key, value]) => [key, kompakt(value)]));
}
