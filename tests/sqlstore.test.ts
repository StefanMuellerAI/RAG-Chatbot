...
describe("Lesende Abfragen", () => {
  it("fuehrt SELECT und WITH aus und begrenzt die Zeilen", () => {
    const summe = runReadOnlyQuery(db, "SELECT kunde, SUM(betrag) AS gesamt FROM umsatz GROUP BY kunde ORDER BY kunde;");
    expect(summe.rows).toEqual([
      ["Alpha", 15.75],
      ["Beta", 20],
    ]);

    const cte = runReadOnlyQuery(db, "WITH a AS (SELECT * FROM umsatz WHERE betrag > 6) SELECT COUNT(*) FROM a");
    expect(cte.rows[0][0]).toBe(2);

    replaceTable(db, "viele", [{ name: "n", type: "INTEGER" }], Array.from({ length: 250 }, (_, i) => [i]));
  ...
  it("laesst Funktionen wie replace() zu, aber keine Schreibbefehle", () => {
    expect(assertReadOnlySql("SELECT replace(kunde, 'a', 'b') FROM umsatz")).toContain("replace(");
    for (const boese of [
      "DROP TABLE umsatz",
  ...
  it("ignoriert Schluesselwoerter in String-Literalen und Kommentaren", () => {
    expect(() => assertReadOnlySql("SELECT 'drop table x' AS t FROM umsatz -- update nichts")).not.toThrow();
  });
